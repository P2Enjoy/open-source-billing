  ...
  module Reports
    class RevenueByClient < Reporting::Report
      ...
      def get_report_data
        ...
        month_wise_payment = []
        (@report_criteria.from_month..@report_criteria.to_month).each { |month| month_wise_payment << "SUM(CASE WHEN strftime('%m', IFNULL(i.due_date, i.invoice_date)) = #{month} THEN i.invoice_total ELSE NULL END) AS #{Date::MONTHNAMES[month]}" }
        ...
        Payment.find_by_sql("
                SELECT case when c.organization_name = '' then (c.first_name || ' ' || c.last_name) else c.organization_name end as organization_name, #{month_wise_payment},
                SUM(i.invoice_total) AS client_total,
                IFNULL(cr.unit,'EUR') AS currency_code,
                ...
                WHERE strftime('%Y', IFNULL(i.due_date, i.invoice_date)) = #{@report_criteria.year}
                      AND strftime('%m', IFNULL(i.due_date, i.invoice_date)) >= #{@report_criteria.from_month} AND strftime('%m', IFNULL(i.due_date, i.invoice_date)) <= #{@report_criteria.to_month}
                      AND i.status <> 'draft'
