  ...
  module Client
    class DashboardController < Portal::Client::BaseController
      ...
      def prepare_multi_currency_charts_data
        invoices_chart_data = @current_client_invoices.where('invoices.invoice_date > ?', 6.months.ago).order('invoice_date asc').group('currencies.unit').group("strftime('%m', invoices.invoice_date)").sum('invoices.invoice_total')
        ...
        payments_chart_data = @current_client_payments.where('payments.created_at > ?', 6.months.ago).order('payments.created_at asc').group('currencies.unit').group("strftime('%m', payments.created_at)").sum('payments.payment_amount')
        currencies = invoices_chart_data.keys.collect{|a| a.first}.uniq
