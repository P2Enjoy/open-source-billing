...
class DashboardController < ApplicationController
  ...
  def prepare_base_currency_charts_data
    @invoices_chart_data_base = @current_company_invoices.where('invoices.invoice_date > ?', 6.months.ago).order('invoice_date asc').group("strftime('%m', invoices.invoice_date)").sum('invoices.base_currency_equivalent_total')
  ...
  def prepare_multi_currency_charts_data
    invoices_chart_data = @current_company_invoices.where('invoices.invoice_date > ?', 6.months.ago).order('invoice_date asc').group('currencies.unit').group("strftime('%m', invoices.invoice_date)").sum('invoices.invoice_total')
    ...
    payments_chart_data = @current_company_payments.where('payments.created_at > ?', 6.months.ago).order('payments.created_at asc').group('currencies.unit').group("strftime('%m', payments.created_at)").sum('payments.payment_amount')
    currencies = invoices_chart_data.keys.collect{|a| a.first}.uniq
    @payments_chart_data = {}
    currencies.each do |currency|
      5.downto(0) do |n|
